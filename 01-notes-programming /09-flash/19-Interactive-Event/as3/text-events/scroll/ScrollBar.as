package {
  ...
  public class ScrollBar extends Sprite {
    ...
    private var t:TextField;                  // The text field to which this scrollbar is applied
    private var tHeight:Number;               // The current height of the text field. If the text field's height changes, we update the height of this scrollbar.
    private var scrollTrack:Sprite;           // The background graphic for the scrollbar
    private var scrollThumb:Sprite;           // The scrollbar's draggable "scroll thumb"
    private var scrollbarWidth:int = 15;      // The scrollbar's width
    private var minimumThumbHeight:int = 10;  // The minimum height of the scrollbar's scroll thumb
    private var dragging:Boolean = false;     // A flag indicating whether the user is currently dragging the scroll thumb
    private var changed:Boolean = false;      // A flag indicating whether the scrollbar should be redrawn at the next scheduled screen update
    
    // Constructor.
    // @param textfield The TextField object to which to apply this scrollbar.
    
	public function ScrollBar (textfield:TextField) {
      
      t = textfield; // Store a reference to the TextField to which this scrollbar is applied
      tHeight = t.height; // Remember the text field's height so that we can track it for changes that require a scrollbar redraw.
      ...
      // Register an Event.SCROLL listener that will update the scrollbar to match the current scroll position of the text field
      t.addEventListener(Event.SCROLL, scrollListener);

      // Register with scrollThumb for mouse down events, which will trigger the dragging of the scroll thumb
	...
	public function updateScrollbar ():void {
      ...
      if (numVisibleLines < t.numLines) {
        ...
        var thumbHeight:int = Math.floor(t.height * (numVisibleLines/t.numLines));
