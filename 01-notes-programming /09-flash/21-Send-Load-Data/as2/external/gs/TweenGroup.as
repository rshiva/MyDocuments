...
dynamic class gs.TweenGroup {
		public static var version:Number = 0.99;
		...
		private var _loopCount:Number; //number of times the group has yoyo'd or loop'd.
...
//---- PUBLIC FUNCTIONS ----------------------------------------------------------------------------------------
		...
		public function reverse($forcePlay:Boolean):Void {
			_reversed = !_reversed;
			var i:Number, tween:Object, initTime:Number, startTime:Number, prog:Number, timeScale:Number, timeOffset:Number = 0, isFinished:Boolean = false;
			...
			for (i = _tweens.length - 1; i > -1; i--) {
				...
				startTime = Math.round(time - ((1 - prog) * tween.duration * 1000 / timeScale) + timeOffset);
				tween.initTime = Math.round(startTime - (tween.delay * (1000 / timeScale)));
...
//---- PRIVATE FUNCTIONS ---------------------------------------------------------------------------------------
		...
		private function offsetTime($tweens:Array, $offset:Number):Void {
			if ($tweens.length != 0) {
				...
				var isPaused:Boolean, tween:TweenLite, renderTime:Number, startTime:Number, end:Number, i:Number, toRender:Array = [];
