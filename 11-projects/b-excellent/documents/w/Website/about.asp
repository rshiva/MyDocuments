...
ElseIf Not IsNumeric(request.QueryString("ID")) Then
    SeoTitle = "参数错误"
ElseIf conn.Execute("select * from Qianbo_About Where ID="&request.QueryString("ID")).EOF Then
    SeoTitle = "参数错误"
Else
    Set rs = server.CreateObject("adodb.recordset")
    sql = "select * from Qianbo_About where ID="&request.QueryString("ID")
...
If request.QueryString("ID") = "" Or Not IsNumeric(request.QueryString("ID")) Then
    sql = "select top 1 * from Qianbo_About where id=1"
Else
    sql = "select * from Qianbo_About where ID="&request.QueryString("ID")
End If
...
%>
              ...
              <a href="?ID=<%=request("ID")%>&page=<%=p%>"><font color="red"><%=p+1%></font></a>
              <%Next%>
              页</p>
    <%End If
rs.Close
