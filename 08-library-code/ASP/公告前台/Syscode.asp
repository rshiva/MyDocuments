...
<%
dim i
if rsBigClass_Down.eof and rsBigClass_Down.bof then
	Response.Write "栏目正在建设中……"
else
	i=1
...
%>
	<TR>
		<TD language=javascript onmouseup="opencat(cat10<%=i%>000,&#13;&#10; img10<%=i%>000);" id=item$pval[catID]) style="CURSOR: hand" width=32 height=24 align=center><IMG id=img10<%=i%>000 src="img/class1.gif" width=20 height=20></TD>
		<TD width="162"><a href='download.asp?BigClassName=<%=rsBigClass_Down("BigClassName")%>'><%=rsBigClass_Down("BigClassName")%></a></TD>
	</TR>
	<TR>
	    <% if BigClassName<>rsBigClass_Down("BigClassName") then %>
		<TD id=cat10<%=i%>000 style="DISPLAY: none" colspan="2">
		<% else %>
		<TD id=cat10<%=i%>000 style="DISPLAY"  colspan="2">
		<% end if %>
...
dim rss,sqls,j
set rss = server.CreateObject ("adodb.recordset")
sqls="select * from SmallClass_Down where BigClassName='" & rsBigClass_Down("BigClassName") & "' order by SmallClassID"
rss.open sqls,conn,1,1
if rss.eof and rss.bof then
	Response.Write "没有小分类"
else
...
sub ProductContent(intTitleLen)
	...
	do while not rsProduct.eof
		...
		strTemp= strTemp & "<table width=100% border=0 cellspacing=3 cellpadding=0>"
                ...
                strTemp= strTemp & "<img style='BORDER-LEFT-COLOR: #cccccc; BORDER-BOTTOM-COLOR: #cccccc; BORDER-TOP-COLOR: #cccccc; BORDER-RIGHT-COLOR: #cccccc' src=" & rsProduct("DefaultPicUrl") & " width=105 height=80>" 
				else
				 if fileext="swf" then
...
sub DownContent(intTitleLen)
	...
	do while not rsDown.eof
		...
		strTemp= strTemp & "<table width=100% border=0 cellspacing=3 cellpadding=0>"
                ...
                strTemp= strTemp & "<td>"
                strTemp= strTemp & "<a href=" & rsDown("DownloadUrl") & "><font color='#FF0000'>点击下载</font><img src='Images/download.gif' width='14' height='14' border='0'></a></td>"
                strTemp= strTemp & "</tr><tr>"
                strTemp= strTemp & "<td colspan=2>"
                strTemp= strTemp & "<table width=100% border=0 cellpadding=0 cellspacing=0>"
                strTemp= strTemp & "<tr>" 
